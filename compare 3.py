...
# ============================================================
# 1) IMPORT PPO CODE (change filename if yours is different)
# ============================================================
...
# ============================================================
# 2) LBO (Kafshdozak) as a clean function (NO globals)
#    returns best dominating-set vector (0/1 list) OR None
# ============================================================
def lbo_find_dominating_set(sensors, model, matrix, params):
        ...
        for i in range(len(sensors) - 1):
            if solution[i] == 1:
                ...
                for nb in sensors[i].neighbors:
                    dominated[nb] = 1
        dominated[-1] = 1  # BS always dominated
    ...
    while NFE < max_NFE:
        ...
        for i in range(npop):
            ...
            if random.random() > 0.2:
                new_pos = (
                    ...
                    + np.random.random(size=nvar)
                    * (np.array(pop[j]["position"]) - np.array(pop[i]["position"]))
                    + np.random.random(size=nvar)
                    * (np.array(pop[j - 1]["position"]) - np.array(pop[j]["position"]))
                )
        ...
        # population reduction (same as original)
...
#    EXACTLY like your original while-loop
# ============================================================
...
# ============================================================
# 4) PPO: build dominating set using trained PPO policy
# ============================================================
...
# ============================================================
# 5) Outer-loop lifetime evaluation for PPO (same as LBO)
# ============================================================
def eval_ppo_lifetime(ppo, runs=30):
    ...
    for _ in range(runs):
        ...
        while not is_finished():
            # reset only active/dominated; energies remain current in sensors
            env.reset()
...
def main():
    ...
    plt.tight_layout()
    plt.savefig("results/ppo_learning_reward.png", dpi=200)
    ...
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig("results/ppo_learning_lifetime.png", dpi=200)
    ...
    plt.tight_layout()
    plt.savefig("results/compare_lifetime_box.png", dpi=200)
    ...
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig("results/compare_lifetime_cdf.png", dpi=200)

    np.savez(
        "results/summary_lifetime.npz",
